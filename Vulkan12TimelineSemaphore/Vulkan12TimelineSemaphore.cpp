...
class VLK
{
	...
	vk::UniqueSemaphore mTimelineSema; // timeline semaphore
	vk::UniqueSemaphore mSwapchainSema; // WSI semaphore cannot use timeline :(
	uint64_t mTimelineSemaCount = 5;
...
public:
	...
	VLK(int width, int height, HWND hwnd)
	{
		...
		/*const*/ auto featureTimelineSema = vk::PhysicalDeviceTimelineSemaphoreFeatures(1);
		const auto enableFeatures2 = vk::PhysicalDeviceFeatures2()
			.setFeatures(vk::PhysicalDeviceFeatures()
							.setSamplerAnisotropy(1))
			.setPNext(&featureTimelineSema);
		...
		const auto semaTypeInfo = vk::SemaphoreTypeCreateInfo(vk::SemaphoreType::eTimeline, mTimelineSemaCount);
		mTimelineSema = mDevice->createSemaphoreUnique(vk::SemaphoreCreateInfo().setPNext(&semaTypeInfo));
		// Increment semaphore counter for next use
		mTimelineSemaCount++;
...
float4 main(Input input) : SV_Target {
	...
	return color;
		...
		auto timelineInfo = vk::TimelineSemaphoreSubmitInfo(0, nullptr, 1, &mTimelineSemaCount);
		const auto submitInfo = vk::SubmitInfo({}, {}, *mDmaCmdBuf, *mTimelineSema)
									.setPNext(&timelineInfo);
		mQueueDma.submit(submitInfo, VK_NULL_HANDLE);

		// Wait a grapihcs queue
		timelineInfo = vk::TimelineSemaphoreSubmitInfo(1, &mTimelineSemaCount, 0, nullptr);
		const vk::PipelineStageFlags submitPipelineStage = vk::PipelineStageFlagBits::eBottomOfPipe;
		const auto submitInfoGfx = vk::SubmitInfo(*mTimelineSema, submitPipelineStage, {}, {})
										.setPNext(&timelineInfo);
		mQueue.submit(submitInfoGfx, VK_NULL_HANDLE);

		// Increment timeline semaphore value
		mTimelineSemaCount++;
	}

	void Draw()
	{
		const uint64_t waitSemaCount = mTimelineSemaCount - 2;
		const auto sema = *mTimelineSema;
		auto vkres = mDevice->waitSemaphores(vk::SemaphoreWaitInfo({}, 1, &sema, &waitSemaCount), 100000000000);
		CHK(vkres);
		...
		*reinterpret_cast<decltype(&wvpMat)>(pUB) = wvpMat;
		...
		const vk::PipelineStageFlags submitPipelineStage = vk::PipelineStageFlagBits::eBottomOfPipe;
		const auto submitInfo = vk::SubmitInfo(*mSwapchainSema, submitPipelineStage, cmdBuf, {});
		mQueue.submit(submitInfo, VK_NULL_HANDLE);

		// Signal a timeline semaphore

		const auto timelineInfo = vk::TimelineSemaphoreSubmitInfo(0, nullptr, 1, &mTimelineSemaCount);
		const auto timelineSubmitInfo = vk::SubmitInfo({}, {}, {}, *mTimelineSema)
											.setPNext(&timelineInfo);
		mQueue.submit(timelineSubmitInfo, VK_NULL_HANDLE);
	}

	void Present()
	{
		// Wait a timeline semaphore

		const vk::PipelineStageFlags submitPipelineStage = vk::PipelineStageFlagBits::eBottomOfPipe;
		const auto timelineInfo = vk::TimelineSemaphoreSubmitInfo(1, &mTimelineSemaCount, 0, nullptr);
		const auto timelineSubmitInfo = vk::SubmitInfo(*mTimelineSema, submitPipelineStage)
											.setPNext(&timelineInfo);
		mQueuePresent.submit(timelineSubmitInfo, VK_NULL_HANDLE);
		mTimelineSemaCount++;

		// Present

		const auto presentInfo = vk::PresentInfoKHR({}, *mSwapchain, mBackBufferIdx);
