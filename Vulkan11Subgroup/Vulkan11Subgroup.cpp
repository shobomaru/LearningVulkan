...
float4 main(Input input) : SV_Target {
	float n = (float)WaveGetLaneIndex() / (float)(WaveGetLaneCount() - 1); // Subgroup instructions
	float4 color = float4(HsvToRgb(float3(n, 1, 1)),  1);
	return color;
}
)#";
		...
		const wchar_t* shaderArgsPS[] = {
			...
			L"-Zi", L"-all_resources_bound", L"-Qembed_debug", L"-spirv", L"-fvk-use-dx-position-w", L"-fspv-target-env=vulkan1.1"
		};
