...
namespace
{
	...
	const int MAX_SHADER_VARIATION = 10;
};
...
class VLK
{
	...
	vk::UniqueShaderModule mSceneVS;
	vk::UniqueShaderModule mSceneFS;
	vk::UniquePipeline mPSO[MAX_SHADER_VARIATION];
...
[[vk::constant_id(0)]] const float HueKey = 0;
float3 HsvToRgb(float3 hsv) {
	return hsv.z * lerp(1.0, saturate(abs(frac(hsv.xxx + float3(1.0, 2.0/3, 1.0/3)) * 6 - 3) - 1), hsv.y);
}
float4 main() : SV_Target {
	float4 color = float4(HsvToRgb(float3(HueKey, 1.0, 1.0)), 1.0);
	return color;
}
)#";
		...
		mSceneVS =  mDevice->createShaderModuleUnique(vsCreateInfo);
		...
		mSceneFS = mDevice->createShaderModuleUnique(psCreateInfo);

		// PSO creation is deferred
	...
	void Draw()
	{
		...
		*reinterpret_cast<decltype(&wvpMat)>(pUB) = wvpMat;
		...
		// Create a PSO with specialization constants
		if (!(mPSO[mConstantIndex]))
		{
			// Calcurate hue
			float hue = (float)mConstantIndex / MAX_SHADER_VARIATION;

			// Set specialization constants
			const auto specialMap = vk::SpecializationMapEntry(
				0, 0, sizeof(float)
			);
			const auto specialInfo = vk::SpecializationInfo(
				1, &specialMap, sizeof(hue), &hue
			);
			const auto pipelineShadersInfo = {
					vk::PipelineShaderStageCreateInfo({}, vk::ShaderStageFlagBits::eVertex, *mSceneVS, "main", &specialInfo),
					vk::PipelineShaderStageCreateInfo({}, vk::ShaderStageFlagBits::eFragment, *mSceneFS, "main", &specialInfo),
			};
			...
			CHK(vkres.result);
			mPSO[mConstantIndex] = std::move(vkres.value);
		}
		...
		cmdBuf.bindPipeline(vk::PipelineBindPoint::eGraphics, *mPSO[mConstantIndex]);
	...
	void IncConstantIndex()
	{
		mConstantIndex++;
		if (mConstantIndex >= MAX_SHADER_VARIATION)
			mConstantIndex = 0;
	}
	void DecConstantIndex()
	{
		mConstantIndex--;
		if (mConstantIndex < 0)
			mConstantIndex = MAX_SHADER_VARIATION - 1;
	}
...
#endif
	{
		...
		while (msg.message != WM_QUIT) {
			...
			if (r == 0) {
				...
				{
					if (msg.wParam == VK_RIGHT) vlk.IncConstantIndex();
					else if (msg.wParam == VK_LEFT) vlk.DecConstantIndex();
				}
			}
		}
	}
