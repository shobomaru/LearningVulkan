...
float4 main(Input input) : SV_Target {
	...
	return color;
		...
		// Imageless, set descriptions of images
		const auto depthFormat = vk::Format::eD32Sfloat;
		const array<vk::FramebufferAttachmentImageInfo, 2> framebufferAttachmentImages = {
			vk::FramebufferAttachmentImageInfo(
				{}, vk::ImageUsageFlagBits::eColorAttachment | vk::ImageUsageFlagBits::eTransferSrc,
				width, height, 1, surfaceFormat),
			vk::FramebufferAttachmentImageInfo(
				{}, vk::ImageUsageFlagBits::eDepthStencilAttachment,
				width, height, 1, depthFormat),
		};
		const auto framebufferAttachmentInfo = vk::FramebufferAttachmentsCreateInfo(
	...
	void Draw()
	{
		...
		*reinterpret_cast<decltype(&wvpMat)>(pUB) = wvpMat;
		...
		const auto renderPassAttachmentInfo = vk::RenderPassAttachmentBeginInfo(framebufferAttachments);
		const auto renderPassInfo = vk::RenderPassBeginInfo(
			*mRenderPass, *mSceneFramebuffer, vk::Rect2D({}, mSceneExtent), sceneClearValue
		).setPNext(&renderPassAttachmentInfo);
