...
public:
	...
	VLK(int width, int height, HWND hwnd)
	{
		...
		/*const*/ auto enableShaderTI = vk::PhysicalDeviceShaderTerminateInvocationFeaturesKHR(1);
		const auto enableFeatures = vk::PhysicalDeviceFeatures2()
			.setFeatures(vk::PhysicalDeviceFeatures()
				.setSamplerAnisotropy(1))
			.setPNext(&enableShaderTI);
		const auto deviceCreateInfo = vk::DeviceCreateInfo(
			{}, (uint32_t)deviceQueueInfos.size(), deviceQueueInfos.data(),
			0, nullptr, _countof(deviceExtensions), deviceExtensions
		).setPNext(&enableFeatures);
...
[[vk::ext_instruction(252)]]
void OpKill();

[[vk::ext_extension("SPV_KHR_terminate_invocation")]]
[[vk::ext_instruction(4416)]]
//[[vk::ext_type_def(1, 4416)]]
void OpTerminateInvocation();

//[[vk::ext_capability(5379)]]
//[[vk::ext_extension("SPV_EXT_demote_to_helper_invocation")]]
//[[vk::ext_instruction(5380)]]
void OpDemoteToHelperInvocationEXT();

[[vk::ext_instruction(131)]]
float2 my_sub(float2 v, float2 w);
...
float4 main(Input input) : SV_Target {
	float2 uv = (float2)0;
	if (((uint)input.position.x ^ (uint)input.position.y) & 1) {
		;
	}
	uv = input.texcoord;
	uv = my_sub(uv, uv); // uv == 0.0
	float4 color = Tex.Sample(SS, uv);
	return color;
}
)#";
		...
		mImageUploadBuffer = mDevice->createBufferUnique(vk::BufferCreateInfo(
			{}, 32 * 1024 * 1024, vk::BufferUsageFlagBits::eTransferSrc
		));
		...
		auto createPlaneColorImage = [&](int w, int h, uint32_t col)
		{
			auto data = make_unique<uint8_t[]>(w * h * 4); // rgba
			for (int y = 0; y < h; y++)
			{
				for (int x = 0; x < w; ++x)
				{
					const int p = y * w + x;
					*reinterpret_cast<uint32_t*>(&data[p * 4]) = col;
				}
			}
			return ImageData{
				vk::Extent3D(w, h, 1),
		...
		vector<ImageData> imageData(7);
		imageData[0] = createPlaneColorImage(1024, 1024, 0xFFFFFFFF);
		imageData[1] = createPlaneColorImage(512, 512, 0xFF0000FF);
		imageData[2] = createPlaneColorImage(256, 256, 0xFF00FFFF);
		imageData[3] = createPlaneColorImage(128, 128, 0xFF00FF00);
		imageData[4] = createPlaneColorImage(64, 64, 0xFFFFFF00);
		imageData[5] = createPlaneColorImage(32, 32, 0xFFFF0000);
		imageData[6] = createPlaneColorImage(16, 16, 0xFFFF00FF);
		const auto& sailboatData = imageData;
		const auto& lennaData = imageData;
