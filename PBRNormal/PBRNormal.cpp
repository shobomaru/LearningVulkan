...
class VLK
{
	...
	vk::UniqueImage mNormalImg;
	vk::UniqueImageView mNormalView;
	uint32_t mNormalMipLevels;
...
Output main(uint instanceID : SV_InstanceID, [[vk::builtin("BaseInstance")]] uint baseInstanceID : BaseInstanceID,
			 float3 position : Position, float3 normal : Normal, float3 tangent : Tangent, float2 texcoord : Texcoord) {
...
float4 main() : SV_Target {
	...
	return float4(color, 1);
		...
		const auto vertexInputBindingDesc
			= vk::VertexInputBindingDescription(0, sizeof(VertexElement), vk::VertexInputRate::eVertex);
		...
		for (int y = 0; y < SphereStacks; ++y)
		{
			for (int x = 0; x < SphereSlices; ++x)
			{
				...
				auto sub_v3 = [](const float a[3], const float b[3], float c[3]) {
					c[0] = a[0] - b[0]; c[1] = a[1] - b[1]; c[2] = a[2] - b[2];
				};
				auto sub_v2 = [](const float a[2], const float b[2], float c[2]) {
					c[0] = a[0] - b[0]; c[1] = a[1] - b[1];
				};
				auto mul_v3 = [](const float a[3], float b, float c[3]) {
					c[0] = a[0] * b; c[1] = a[1] * b; c[2] = a[2] * b;
				};
				auto v0 = getVertex(x, y);
				auto v1 = getVertex(x + 1, y);
				auto v2 = getVertex(x, y + 1);
				auto v3 = getVertex(x + 1, y + 1);
				const short b = static_cast<short>(vertices.size());
				IndexList il = { b, b + 2, b + 1, b + 2, b + 3, b + 1 };
				indices.push_back(il);
				...
				if (fabs(v1.position[0] - v0.position[0]) > 0.0001f
					...
					sub_v2(v1.texcoord, v0.texcoord, deltaUV2);
				} else {
					sub_v3(v1.position, v2.position, deltaPos1);
					sub_v3(v3.position, v2.position, deltaPos2);
					sub_v2(v1.texcoord, v2.texcoord, deltaUV1);
					sub_v2(v3.texcoord, v2.texcoord, deltaUV2);
				}
				float r = 1.0f / (deltaUV1[0] * deltaUV2[1] - deltaUV1[1] * deltaUV2[0]);
				float temp1[3], temp2[3], temp3[3], t[3];
				mul_v3(deltaPos1, deltaUV2[1], temp1);
				mul_v3(deltaPos2, deltaUV1[1], temp2);
				sub_v3(temp1, temp2, temp3);
				mul_v3(temp3, r, t);
				copy(cbegin(t), cend(t), v0.tangent);
				copy(cbegin(t), cend(t), v1.tangent);
				copy(cbegin(t), cend(t), v2.tangent);
				copy(cbegin(t), cend(t), v3.tangent);
		...
		for (auto& v : vertices)
		{
			auto p = DirectX::XMFLOAT3(v.tangent);
			auto t = DirectX::XMVector3Normalize(DirectX::XMLoadFloat3(&p));
		...
		auto normalData = generateMipmap(loadBitmap("../res/Brick_Wall_017_normal.bmp"));
		mSailboatMipLevels = (uint32_t)sailboatData.size();
		mLennaMipLevels = (uint32_t)lennaData.size();
		mNormalMipLevels = (uint32_t)normalData.size();
		...
		imgMemOffset = ALIGN(imgMemOffset + lennaMemReq.size, normalMemReq.alignment);
		mDevice->bindImageMemory(*mNormalImg, *mImagesMemory, imgMemOffset);
		...
		const auto barriers = {
			...
			vk::ImageMemoryBarrier(
				{}, vk::AccessFlagBits::eTransferWrite,
				vk::ImageLayout::eUndefined, vk::ImageLayout::eTransferDstOptimal,
				mQueueFamilyDmaIdx, mQueueFamilyDmaIdx,* mNormalImg,
				vk::ImageSubresourceRange(vk::ImageAspectFlagBits::eColor, 0, mNormalMipLevels, 0, 1)
			),
		};
		...
		{
			mDmaCmdBuf->copyBufferToImage(
				*mImageUploadBuffer, *mNormalImg, vk::ImageLayout::eTransferDstOptimal,
				vk::BufferImageCopy(
					bufferOffset, 0, 0,
					vk::ImageSubresourceLayers(vk::ImageAspectFlagBits::eColor, i, 0, 1), {}, normalData[i].extent));
			bufferOffset += normalData[i].size;
		}
		...
		const auto barriers2 = {
			...
			vk::ImageMemoryBarrier(
				vk::AccessFlagBits::eTransferWrite, {},
				vk::ImageLayout::eTransferDstOptimal, vk::ImageLayout::eShaderReadOnlyOptimal,
				mQueueFamilyDmaIdx, mQueueFamilyGfxIdx, *mNormalImg,
				vk::ImageSubresourceRange(vk::ImageAspectFlagBits::eColor, 0, mNormalMipLevels, 0, 1)
			),
		};
		mDmaCmdBuf->pipelineBarrier(
