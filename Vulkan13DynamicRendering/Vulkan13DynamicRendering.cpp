...
public:
	...
	VLK(int width, int height, HWND hwnd)
	{
		...
		const auto appInfo = vk::ApplicationInfo(
			...
			"LearningVulkan", VK_MAKE_API_VERSION(0, 0, 0, 0), VK_API_VERSION_1_3);
		...
		auto enableDynamicRendering = vk::PhysicalDeviceDynamicRenderingFeatures(1);
...
float4 main(Input input) : SV_Target {
	...
	return color;
		...
		const auto pipelineRenderingInfo = vk::PipelineRenderingCreateInfo(
			0, surfaceFormat, vk::Format::eD32Sfloat, vk::Format::eUndefined
		);
		const auto pipelineInfo = vk::GraphicsPipelineCreateInfo(
			...
			&pipelineBSInfo, &pipelineDynamicStatesInfo, *mPipelineLayout, VK_NULL_HANDLE
	...
	void Draw()
	{
		...
		*reinterpret_cast<decltype(&wvpMat)>(pUB) = wvpMat;
		...
		// Dynamic rendering have no initialLayout, so we insert barriers manually
		const bool isFirstTimeRendering = (mFrameCount == 1);
		const auto barrier = vk::ImageMemoryBarrier(
			vk::AccessFlagBits::eNone,
			vk::AccessFlagBits::eColorAttachmentWrite,
			isFirstTimeRendering ? vk::ImageLayout::eUndefined : vk::ImageLayout::eTransferSrcOptimal,
		...
		{
			auto barrierDepth = vk::ImageMemoryBarrier(
				vk::AccessFlagBits::eNone,
				vk::AccessFlagBits::eDepthStencilAttachmentWrite,
				isFirstTimeRendering ? vk::ImageLayout::eUndefined : vk::ImageLayout::eDepthStencilAttachmentOptimal,
		...
		const auto colorAttachmentInfo = vk::RenderingAttachmentInfo(
			*mSceneColorView, vk::ImageLayout::eColorAttachmentOptimal, {}, {}, {},
			vk::AttachmentLoadOp::eClear, vk::AttachmentStoreOp::eStore, sceneClearValue[0]
		);
		const auto depthAttachmentInfo = vk::RenderingAttachmentInfo(
			*mSceneDepthView, vk::ImageLayout::eDepthStencilAttachmentOptimal, {}, {}, {},
			vk::AttachmentLoadOp::eClear, vk::AttachmentStoreOp::eDontCare, sceneClearValue[1]
		);
		const auto renderingInfo = vk::RenderingInfo(
			{}, vk::Rect2D({}, mSceneExtent), 1, 0,
			colorAttachmentInfo, &depthAttachmentInfo, VK_NULL_HANDLE
		);
		...
		// Blit to back buffer
		const array<vk::ImageMemoryBarrier, 2> barriers = {
			// Dynamic rendering have no finalLayout, so we insert a barrier manually
